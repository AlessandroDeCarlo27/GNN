...
def scatterplot(y_real_test,y_pred_test,nameVar,idxFig):
    ...
    plt.savefig('PredVsObs_'+nameVar+str(idxFig)+'.png', bbox_inches='tight', dpi=250)
    plt.clf()
...
def residuals(y_real_test,res,nameVar,idxFig):
    ...
    plt.savefig('Res_'+nameVar+str(idxFig)+'.png', bbox_inches='tight', dpi=250)
    plt.clf()
...
def store_recap_test_class(buffer_auprc,buffer_auroc,filename):
    N = len(buffer_auprc)
    idx_list = np.arange(1,N+1,1).tolist()
    if len(idx_list)==1:
        idx_list[0] = 'Hold Out'
    recap_df = pd.DataFrame(list(zip(idx_list,buffer_auprc,buffer_auroc)),
                            columns=['Fold','AUPRC','AUROC'])
...
def perform_validation_class(model,validation,val_metric,batchsize,nfeat):
    ...
    for b_val in batch_val:
        ...
        if y_pred_val.ndim == 0:
            y_pred_val = tf.reshape(y_pred_val,shape=(1,))
        y_pred_val = tf.nn.sigmoid(y_pred_val)
        val_metric.update_state(y_real_val,y_pred_val)
    return val_metric.result().numpy()

def plot_ROC(Y_true, Y_pred_prob, nameVar,idxFig):
    ...
    plt.xlabel('FPR')
    plt.ylabel('TPR')
    plt.title(nameVar + " ROC Curve")
    plt.savefig('ROC' + str(idxFig) + '.png', bbox_inches='tight', dpi=250)
    plt.clf()

def plot_PRC(Y_true, Y_pred_prob, nameVar,idxFig):
    ...
    plt.title(nameVar + " PR Curve")
    plt.savefig('PR' + str(idxFig) + '.png', bbox_inches='tight', dpi=250)
    plt.clf()

def perform_validation_class_mt(model,validation,val_metric,batchsize,nfeat):
    ...
    for b_val in batch_val:
        ...
        y1_real_val = bv.Y1.to_numpy()
        y2_real_val = bv.Y2.to_numpy()
        y3_real_val = bv.Y3.to_numpy()
        maxDim_nodes_val, maxDim_links_val = get_maxDims(bv,nfeat)
        for smile_val in bv.SMILES:
            feat_val, NmatList_val, CmatList_val, mask_val, mask_list_mol,Nall_mol,Call_mol = gen_smiles2graph(smile_val,nfeat)
            if batchsize>1: #PADDING
        ...
        y1_pred_val, y2_pred_val, y3_pred_val = tf.squeeze(model((feat_batch_val,
                                       nm_batch1_val,cm_batch1_val,
                                       nm_batch2_val, cm_batch2_val,
                                       nm_batch3_val, cm_batch3_val,
                                       nm_batch4_val, cm_batch4_val,
                                       ...
                                       nmall_batch, cmall_batch,
                                       mask_batch_val), training=False))
        if y1_pred_val.ndim == 0:
            y1_pred_val = tf.reshape(y1_pred_val,shape=(1,))
            y2_pred_val = tf.reshape(y2_pred_val, shape=(1,))
            y3_pred_val = tf.reshape(y3_pred_val, shape=(1,))
        y1_pred_val = tf.nn.sigmoid(y1_pred_val)
        y2_pred_val = tf.nn.sigmoid(y2_pred_val)
        y3_pred_val = tf.nn.sigmoid(y3_pred_val)
        val_metric[0].update_state(y1_real_val,y1_pred_val)
        val_metric[1].update_state(y2_real_val, y2_pred_val)
        val_metric[2].update_state(y3_real_val, y3_pred_val)
    return [vm.result().numpy() for vm in val_metric]

def perform_test_mt(model,validation,batchsize,nfeat):
    ...
    for b_val in batch_val:
        ...
        y1_pred_val,y2_pred_val,y3_pred_val = tf.squeeze(model((feat_batch_val,
                                       ...
                                       nmall_batch, cmall_batch,
                                       mask_batch_val), training=False),axis=[-1,-2])
        pred_list1.append(y1_pred_val)
        pred_list2.append(y2_pred_val)
        pred_list3.append(y3_pred_val)
    return [tf.concat(pred_list1,axis=0).numpy(), tf.concat(pred_list2,axis=0).numpy(), tf.concat(pred_list3,axis=0).numpy()]

def perform_test_mt_regress2(model,validation,batchsize,nfeat):
    ...
    for b_val in batch_val:
        ...
        y1_pred_val,y2_pred_val= tf.squeeze(model((feat_batch_val,
                                       nm_batch1_val, cm_batch1_val,
                                       nm_batch2_val, cm_batch2_val,
                                       nm_batch3_val, cm_batch3_val,
                                       nm_batch4_val, cm_batch4_val,
        ...
        pred_list1.append(y1_pred_val)
        pred_list2.append(y2_pred_val)
    return [tf.concat(pred_list1,axis=0).numpy(), tf.concat(pred_list2,axis=0).numpy()]


def perform_validation_mt_regress2(model,validation,val_metric,batchsize,nfeat,id_task):
    ...
    for b_val in batch_val:
        ...
        y_pred_val1, y_pred_val2 = tf.squeeze(model((feat_batch_val,
                                       nm_batch1_val,cm_batch1_val,
                                       nm_batch2_val, cm_batch2_val,
                                       nm_batch3_val, cm_batch3_val,
                                       nm_batch4_val, cm_batch4_val,
