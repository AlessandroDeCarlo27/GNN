...
dataset = pd.read_csv('AqSolDB.csv',sep=",").dropna().reset_index(drop=True)
...
for train_index,test_index in KFold(NSPLIT,shuffle=True,random_state=12345).split(dataset):
    ...
    ex_scores = np.exp(kde.score_samples(np.linspace(-13, 3, 500)[:, np.newaxis]))
    # create object WeightTrain with the given kernel
    weight_generator = WeightTrain(train.Y.to_numpy(), kde, 0.55)
    scores_train = weight_generator.getWeigths(np.linspace(-13, 3, 500)[:, np.newaxis])
    range = (-13,3)
